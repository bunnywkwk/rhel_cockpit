...
This role keeps the hypervisor headless (no desktop GUI), enables Systemd Socket Activation on TCP port 9090, and applies the session idle timeout required by CIS Benchmark Level 1.
...
- **Firewall**: not managed by this role. The `cockpit` service is already allowed in firewalld's `public` zone on stock RHEL 9 and 10, and the CIS roles do not remove it (see docs/TASK_WALKTHROUGH.md, section 6).
...
# 3. Verify the firewall allows Cockpit (allowed by default on RHEL)
